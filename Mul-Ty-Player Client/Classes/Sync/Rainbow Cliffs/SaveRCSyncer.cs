...
internal class SaveRCSyncer : SaveDataSyncer
{
    ...
    {
        var address = SyncHandler.SaveDataBaseAddress + 0xA84 + iAttribute;
        ProcessHandler.WriteData(address, new byte[] { 1 },
            $"Setting RC data attribute {Enum.GetValues(typeof(RCData)).GetValue(iAttribute)} to true in save data");
    }
