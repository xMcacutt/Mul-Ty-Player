...
internal class SyncHandler
{
    ...
    public static MushroomSyncer SMushroom;
    ...
    public SyncHandler()
    {
        Syncers = new Dictionary<string, Syncer>
        {
            ...
            { "Mushroom", SMushroom = new MushroomSyncer() }
        };
    ...
    private static void HandleServerDataUpdate(ushort fromClientId, Message message)
    {
        ...
        if (SettingsHandler.GameMode == GameMode.Collection)
            CollectionModeHandler.HandleScoreUpdate(fromClientId, syncMessage.level, syncMessage.type, syncMessage.iLive, syncMessage.iSave);
    }
}
