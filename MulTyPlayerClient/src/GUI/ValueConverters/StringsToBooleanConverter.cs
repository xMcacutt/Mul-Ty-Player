...
namespace MulTyPlayerClient.GUI
{
    public class StringsToBooleanConverter : IMultiValueConverter
    {
        ...
        {
            int i = 0;
            foreach (string fileName in SetupViewModel.TyFileNames)
            {
                string filePath = System.IO.Path.Combine(tyPath, fileName);
                FileInfo fileInfo = new FileInfo(filePath);
                int index = Array.IndexOf(SetupViewModel.TyFileNames, fileName);
                if (fileInfo.Exists)
                {
                    i++;
                }
            }
            if (i == SetupViewModel.TyFileNames.Length)
            {
                return true;
            }
            return false;
