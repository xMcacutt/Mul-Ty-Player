...
namespace MulTyPlayerClient.Sync
{
    internal abstract class SyncObjectHandler
    {
        ...
        public int CounterByteLength
        {
            get; set;
        }
        public bool CounterAddressStatic
        {
            get; set;
        }
        public bool SeparateID
        {
            get; set;
        }
        public int IDOffset
        ...
        public byte[] CurrentObjectData
        {
            get; set;
        }
        public byte[] PreviousObjectData
        {
            get; set;
        }
        ...
        public int PreviousObserverState
        {
            get; set;
        }
        ...
        public virtual void HandleClientUpdate(int iLive, int iSave, int level)
        {
            ...
            if (level != LevelHandler.CurrentLevelId)
                return;
            ...
            for (int iLive = 0; iLive < CurrentObjectData.Length; iLive++)
            {
                if (CheckObserverCondition(PreviousObjectData[iLive], CurrentObjectData[iLive]))
                {
                    ...
                    if (GlobalObjectData[LevelHandler.CurrentLevelId][iLive] != CheckState)
                    {
                        ...
                        Replication.HSync.SendDataToServer(iLive, iSave, LevelHandler.CurrentLevelId, Name);
                    }
                }
            }
        }
        ...
        {
            CurrentObjectData = GlobalObjectData[LevelHandler.CurrentLevelId];
            PreviousObjectData = GlobalObjectData[LevelHandler.CurrentLevelId];
        }
    }
}
